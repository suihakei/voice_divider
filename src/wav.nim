...
proc divideBySilence*(wav: WAV, threshold: float, silenceTime: float): seq[WAV] =
    ##
    ## 無音部分でデータを分割します
    ##
    ## wav WAV: WAV構造体を指定します
    ## threshold float: 無音と識別するためのしきい値（0.0～1.0）
    ## silenceTime float: ここで指定した秒数無音が続けば分割を行う秒数
    ...
    for i in wav.data:
        ...
        # 無音がN秒以上続いたらそこでカットする
        if float(silenceCount) >= wav.fmtBytesPerSec / wav.fmtBlockSize * silenceTime:
...
proc trimSilence*(wav: WAV, threshold: float, silenceTime: float, leaveTopTime: float): WAV =
    ##
    ## wavデータの先頭の無音をカットします
    ##
    ## wav WAV: WAV構造体を指定します
    ## threshold float: 無音と識別するためのしきい値（0.0～1.0）
    ## silenceTime float: 無音ごこの秒数続くとカットするというしきいの時間
    ...
    if float(tmp.len) < silenceTime * float(wav.fmtBytesPerSec):
